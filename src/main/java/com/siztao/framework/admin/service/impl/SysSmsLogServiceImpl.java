...
import com.siztao.framework.admin.dao.SysSmsLogDao;
import com.siztao.framework.admin.entity.SmsConfig;
import com.siztao.framework.admin.entity.SysSmsLogEntity;
import com.siztao.framework.admin.entity.SysUserEntity;
import com.siztao.framework.admin.service.SysConfigService;
import com.siztao.framework.admin.service.SysSmsLogService;
...
public class SysSmsLogServiceImpl implements SysSmsLogService {
    ...
    public SysSmsLogEntity sendSms(SysSmsLogEntity smsLog) {
        ...
        try {
            /**
             * result：状态,发送编号,无效号码数,成功提交数,黑名单数和消息，无论发送的号码是多少，一个发送请求只返回一个sendid，如果响应的状态不是“0”，则只有状态和消息
             */
            result = SmsUtil.crSendSms(config.getDomain(), config.getName(), config.getPwd(), smsLog.getMobile(), smsLog.getContent(), config.getSign(),
        ...
        try {
            smsLog.setUserId(((SysUserEntity)ShiroUtils.getUserEntity()).getUserId());
        } catch (Exception e) {
